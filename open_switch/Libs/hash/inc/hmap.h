...
#define ASSIGN_CONTAINER(o,p,m) (o = container_of(p,typeof(*o),m))
...
#define HMAP_CONST(HMAP, N, NODE) {                                 \
        (struct hmap_node **) &(HMAP)->one, NODE, 0, N }
...
#define HMAP_FOR_EACH_SAFE(NODE, NEXT, MEMBER, HMAP) \
    for (ASSIGN_CONTAINER(NODE, hmap_first(HMAP), MEMBER);   \
         (NODE? ASSIGN_CONTAINER(NEXT, hmap_next(HMAP, &(NODE)->MEMBER), MEMBER), 1 \
          : 0);                                                         \
         (NODE) = (NEXT))
...
#define HMAP_FOR_EACH_POP(NODE, MEMBER, HMAP)                               \
    for (size_t bucket__ = 0;                                               \
         ASSIGN_CONTAINER(NODE, hmap_pop_helper__(HMAP, &bucket__), MEMBER),  \
         NODE;)
...
static inline struct hmap_node *
hmap_next_with_hash__(struct hmap_node *node, size_t hash)
{
